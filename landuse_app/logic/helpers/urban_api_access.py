...
async def get_functional_zones_scen_id(scenario_id: int):
    response = await urban_db_api.get(f"/scenarios/{scenario_id}/functional_zones?year=2024&source=OSM")
    if not response or "features" not in response or not response["features"]:
        raise http_exception(404, "No functional zones found for the given project ID:", scenario_id)
    return response

async def get_all_physical_objects_geometries_scen_id(scenario_id: int) -> dict:
    """
    Fetches all physical object geometries for a project, optionally for context.
    ...
    Raises:
        HTTPException: If the response is empty.
    """
    response = await urban_db_api.get(f"/scenarios/{scenario_id}/geometries_with_all_objects")
    try:
        if not response or not response.get("features"):
            raise http_exception(404, "No geometries found for the given project ID:", scenario_id)
    except Exception:
        raise http_exception(500, "Failed to fetch data from urban API", response.status)
    return response
